...
def collate_batch(batch:, collate_columns:, tokenizer:, sts:bool=False):
  ...
  This function takes a batch of data and preprocesses it for use with a neural network.
...
class SBETOnli(nn.Module):
  ...
  NilsReimers and IrynaGurevych. 2019. 
...
class SBETOsts(nn.Module):
  ...
  def forward(self, sentenceA:torch.Tensor, sentenceB:torch.Tensor, att_A:torch.Tensor, att_B:torch.Tensor):
    """
